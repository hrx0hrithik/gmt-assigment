...
const Register = () => {
  ...
  const handleRegister = async (e) => {
    e.preventDefault()
    if (email.length !== 0 && password.length !== 0 && username.length !== 0 && tncChecked) {
      try {
  ...
  return (
    <div className=" flex flex-col h-full p-5 relative mx-1">
      <div className=' w-[327px] py-4'>
        <h1 className=' text-[32px] leading-10 font-semibold text-[#101010] mb-2'>Create your new account</h1>
        <p className=' text-sm font-medium text-[#878787]'>Create an account to start looking for the food you like </p>
      </div>
      <form>
        ...
        <div className=' mb-[14px]'>
          <label className='text-sm font-medium text-[#101010]' htmlFor='email'>User Name</label><br />
          <input
            className=' mt-2 p-4 border w-full rounded-lg h-[52px] text-sm font-medium'
            name='username'
            type="text"
        ...
        <div className=' flex items-start my-4'>
          <input
           ...
           className=' mr-3 mt-1 accent-[#FE8C00] text-white' />
          <div className='text-sm font-medium w-[298px]'>I Agree with <span className=' font-semibold text-[#FE8C00] cursor-pointer'>Terms of Service</span> and <span className=' font-semibold text-[#FE8C00] cursor-pointer'>Privacy Policy</span> </div>
        </div>
      </form>
      <button
        onClick={handleRegister}
        disabled={buttonInactive}
        className={`bg-[#FE8C00] text-white text-sm font-semibold text-center w-full h-[52px] py-4 my-2 rounded-[100px] ${buttonInactive ? 'opacity-50' : 'opacity-100'}`}>Register</button>
      <div className=' my-2 h-5 flex justify-center items-center'>
      ...
      <div className='w-full mt-1 flex justify-center items-center'>
        <button onClick={handleGoogleSignIn} className="h-10 w-10 flex bg-white border-2 border-[#D6D6D6] rounded-full items-center justify-center">
          <img src={googleIcon} alt="" className='w-6 h-6 object-cover origin-center' />
        </button>
      </div>
      <div className=' mt-4 text-sm text-center'>
        <p className=' font-normal'>Have an account? <Link className=' text-[#FE8C00] font-semibold' to="/login">Sign in</Link></p>
      </div>
    </div>
  )
}
